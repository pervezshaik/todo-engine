...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "STATUS: failed — missing capability: sms",
            False,
            "STATUS: failed — missing capability: sms",
        ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            False,
            "STATUS: failed — missing capability: sms",
            TaskOutcome.MISSING_CAPABILITY,
        ),
        (
            False,
            "success",
            False,
            "STATUS: failed — user declined the command",
        ...
        (
            ...
            "success",
            False,
            "STATUS: failed — permission denied for rm",
            TaskOutcome.DECLINED,
        ),
...
def test_classify(
    is_error: bool, subtype: str, status_ok: bool, status_line: str, expected: TaskOutcome
) -> None:
...
async def test_run_task_success_collects_transcript_and_options(
    ...
    ctx = TaskContext(
        workdir=tmp_path,
        extra_allowed_tools=["mcp__local__example"],
        max_turns=7,
...
async def test_run_task_yolo_uses_bypass_permissions(
    tmp_path: Path, fake_sdk: SimpleNamespace
) -> None:
...
async def test_run_task_max_turns_is_timeout(tmp_path: Path, fake_sdk: SimpleNamespace) -> None:
    fake_sdk.agent.push(
        assistant(text_block("still working...")), result(subtype="error_max_turns")
    )
    res = await run_task(make_task(), TaskContext(workdir=tmp_path))
    assert res.outcome is TaskOutcome.TIMEOUT


async def test_run_task_without_status_line_fails(
    tmp_path: Path, fake_sdk: SimpleNamespace
) -> None:
...
async def test_run_task_retry_attempt_is_noted_in_transcript(
    tmp_path: Path, fake_sdk: SimpleNamespace
) -> None:
    fake_sdk.agent.push(*agent_done())
